...
impl IndexerService {
    ...
    pub async fn verify_filesystem_consistency(
        &self,
        share_id: &ShareId,
        ...
        let entries = self.file_index_repo.find_all_by_share(share_id).await?;
        let share_root = std::path::PathBuf::from(&share.local_path);
        ...
        for entry in entries {
            ...
            let abs_path = share_root.join(&entry.path);
            if !abs_path.exists() {
                let deleted_entry = entry.mark_deleted(&self.local_device_id);
                self.file_index_repo.save(&deleted_entry).await?;
                marked_deleted += 1;
                ...
                if meta.len() != entry.size {
                    ...
                    let (size, sha256) = tokio::task::spawn_blocking(move || -> Result<(u64, String), DomainError> {
                    ...
                    e.modified_by = device_id;
                    self.file_index_repo.save(&e).await?;
                    updated += 1;
                }
            }
        }

        if marked_deleted > 0 || updated > 0 {
            println!(
                "(ConsistencyCheck) share={}: {} files marked deleted, {} files updated",
                share_id.0, marked_deleted, updated
            );
        }

        Ok((marked_deleted, updated))
    }
    ...
    async fn process_events(
        ...
        while let Some(event) = rx.recv().await {
            ...
            let relative_path = event.path.strip_prefix(share_root).unwrap_or(&event.path);
            ...
            let existing_entry = file_index_repo
                .find_by_path(share_id, &path_str)
                .await
                .unwrap_or(None);
            ...
            match event.event_type {
                FileEventType::Created | FileEventType::Modified => {
                    ...
                    match computed {
                        ...
                        Err(e) => {
                            eprintln!(
                                "[IndexerService] Skipping file '{}' due to hash error: {}",
                                path_str, e
                            );
                        }
                    }
